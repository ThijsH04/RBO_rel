...
dist <- c(
  `0` = 0.7852387026664488,
  `1` = 0.14289261829010233,
  `2` = 0.0533714244477885,
  `3` = 0.01008806094012323,
  `4` = 0.008409193655537184
)

d <- future_lapply(1:100000, future.seed = 0, function(i) {
  len <- sample(10:100, 2, replace = TRUE)
  xy <- simulate_rankings(min(len), max(len), FALSE,
                          n = 1000, tau = runif(1,.5,1))
  
  # Values and their probabilities
  values <- as.integer(names(dist))
  probs <- as.numeric(dist)
  
  # Sample 1000 values
  samples <- sample(values, size = 1000, replace = TRUE, prob = probs)
  
  # Assign to variables i1 to i1000 (as a list or environment)
  sample_list <- setNames(as.list(samples), paste0("i", 1:1000))
  
  x <- xy$x
  y <- xy$y
  
  x_mapped <- lapply(x, function(group) sample_list[group])
  y_mapped <- lapply(y, function(group) sample_list[group])
  
  # If you need numeric vectors instead of lists of lists
  x_mapped <- lapply(x_mapped, unlist)
  y_mapped <- lapply(y_mapped, unlist)
  
  # Collapse each group of x/y into a single ranking string
  x_str <- paste(unlist(x_mapped), collapse = ",")
  y_str <- paste(unlist(y_mapped), collapse = ",")
  
  x_id_str <- paste(unlist(x), collapse = ",")
  y_id_str <- paste(unlist(y), collapse = ",")
  
  # Return as a one-row data.frame
  data.frame(
    iteration = i,
    x_ranking_id = x_id_str,
    y_ranking_id = y_id_str,
    x_ranking_rel = x_str,
    y_ranking_rel = y_str,
...
# Combine all rows
all_rankings <- do.call(rbind, d)

write.csv(all_rankings, "C:/Users/Thijs/TU Delft/RP/simulated-data/rankings_id_and_relevance.csv", row.names = FALSE)
